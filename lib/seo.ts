import type { AdminSeo, SeoSchemaType } from "./admin-data";

/**
 * Build a page <title> from the template defined in SeoConfig.
 * Template tokens: %title%, %sitename%, %separator%
 */
export function buildTitle(pageTitle: string | undefined, seo: AdminSeo): string {
  const tpl = seo.meta.titleTemplate || "%title% | %sitename%";
  const sep = seo.meta.titleSeparator || "|";
  const sitename = seo.general.siteName || "منصة قانوني";
  if (!pageTitle) return seo.meta.defaultTitle || sitename;
  const out = tpl
    .replace(/%title%/g, pageTitle)
    .replace(/%sitename%/g, sitename)
    .replace(/%separator%/g, sep);
  return seo.meta.autoTrimDescription ? out.slice(0, 60) : out;
}
...
export function absoluteUrl(path: string, seo: AdminSeo): string {
  const root = seo.meta.metadataBase || "https://iqlegal.example";
...
export function organizationSchema(seo: AdminSeo) {
  const g = seo.general;
  return {
    "@context": "https://schema.org",
    "@type": "Organization",
    name: g.companyName || g.siteName,
    url: absoluteUrl("/", seo),
    logo: absoluteUrl(g.logo || g.defaultImage, seo),
    sameAs: Object.values(g.socials).filter(Boolean),
    contactPoint: {
      "@type": "ContactPoint",
      telephone: g.phone,
      email: g.email,
      contactType: "customer service",
      areaServed: g.country,
    },
  };
}

export function legalServiceSchema(seo: AdminSeo) {
  const g = seo.general;
  const l = seo.local;
  return {
    "@context": "https://schema.org",
    ...
    url: absoluteUrl("/", seo),
    image: absoluteUrl(g.defaultImage, seo),
    areaServed: l.governorates,
    availableChannel: {
      "@type": "ServiceChannel",
      serviceLocation: {
        "@type": "Place",
        address: { "@type": "PostalAddress", addressCountry: g.country, streetAddress: g.address },
        geo: { "@type": "GeoCoordinates", latitude: g.latitude, longitude: g.longitude },
      },
    },
    provider: { "@type": "Organization", name: g.companyName },
  };
}

export function websiteSchema(seo: AdminSeo) {
  return {
    "@context": "https://schema.org",
    "@type": "WebSite",
    name: seo.general.siteName,
    url: absoluteUrl("/", seo),
    inLanguage: seo.general.language,
    potentialAction: {
      "@type": "SearchAction",
      target: {
        "@type": "EntryPoint",
        urlTemplate: absoluteUrl("/search?q={search_term_string}", seo),
      },
...
export function breadcrumbSchema(items: { name: string; url: string }[], seo: AdminSeo) {
  return {
    "@context": "https://schema.org",
    "@type": "BreadcrumbList",
...
export function articleSchema(input: {
  ...
  return {
    ...
    image: input.image ? absoluteUrl(input.image, seo) : undefined,
    datePublished: input.datePublished,
    dateModified: input.dateModified,
    author: { "@type": "Person", name: input.author || seo.meta.authorMeta },
    publisher: {
      "@type": "Organization",
      name: seo.general.companyName,
      logo: { "@type": "ImageObject", url: absoluteUrl(seo.general.logo, seo) },
    },
    mainEntityOfPage: { "@type": "WebPage", "@id": absoluteUrl(input.url, seo) },
  };
}

export function attorneySchema(lawyer: {
  name: string;
  ...
  return {
    ...
    image: lawyer.image ? absoluteUrl(lawyer.image, seo) : undefined,
    url: absoluteUrl(lawyer.url, seo),
    telephone: lawyer.phone,
    email: lawyer.email,
    address: { "@type": "PostalAddress", addressLocality: lawyer.city, addressCountry: seo.general.country },
    areaServed: lawyer.city,
  };
}

/** Return the list of enabled schema @types for JSON-LD injection. */
export function enabledSchemaTypes(seo: AdminSeo): SeoSchemaType[] {
  return (Object.keys(seo.schema.enabled) as SeoSchemaType[]).filter(
    (k) => seo.schema.enabled[k]
  );
}
