...
export async function GET(
  ...
  const item = await prisma.cassationDecision.findUnique({ where: { id } });
  if (!item) {
...
export async function PUT(
  ...
  const item = await prisma.cassationDecision.update({
    where: { id },
    data: {
...
export async function DELETE(
  ...
  const { id } = await params;
  await prisma.cassationDecision.delete({ where: { id } });
