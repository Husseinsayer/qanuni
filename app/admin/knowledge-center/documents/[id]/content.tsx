...
import { type RequiredDocument } from "@/lib/knowledge-center/types";
import { ChevronRight, ArrowLeft } from "lucide-react";

export default function DocumentDetailPage() {
  const { id } = useParams<{ id: string }>();
  const router = useRouter();
  const [item, setItem] = useState<RequiredDocument | null>(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    const items = getItems("documents", true);
  ...
  if (!item) return <div className="py-20 text-center"><p className="mb-4 text-muted-foreground">لم يتم العثور على المستند</p><Button onClick={() => router.push("/admin/knowledge-center/documents")}><ArrowLeft className="ml-2 h-4 w-4" /> العودة</Button></div>;
  ...
  return (
    <div>
      <div className="mb-4 flex items-center gap-2 text-sm text-muted-foreground">
        ...
        <Link href="/admin/knowledge-center/documents" className="hover:text-accent">المستندات المطلوبة</Link><ChevronRight className="h-3.5 w-3.5" />
        <span className="text-foreground font-medium truncate max-w-[200px]">{item.name}</span>
      </div>
      <div className="mb-6"><h1 className="text-2xl font-bold">{item.name}</h1></div>

      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <Card className="lg:col-span-1">
          <CardHeader><CardTitle className="text-base">معلومات المستند</CardTitle></CardHeader>
      ...
      <div className="mt-6"><Button variant="outline" onClick={() => router.push("/admin/knowledge-center/documents")}><ArrowLeft className="ml-2 h-4 w-4" /> العودة</Button></div>
    </div>
  );
}
