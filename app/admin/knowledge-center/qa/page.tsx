...
import { type KnowledgeQA, QA_CATEGORIES } from "@/lib/knowledge-center/types";
...
const emptyForm: Omit<KnowledgeQA, "id" | "createdAt" | "updatedAt" | "deletedAt"> = {
  question: "", answer: "", category: QA_CATEGORIES[0], keywords: [], importance: "medium", notes: "", isActive: true,
};
...
export default function QAPage() {
  ...
  const handleImport = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const result = importFromJSON(content, "qa");
  ...
  const reload = () => setItems(getItems("qa", showArchived));
  useEffect(() => { reload(); }, [showArchived]);

  const filtered = showArchived ? items : items.filter((i) => !i.deletedAt);
  const { search, setSearch, page, setPage, paged, totalPages, total } = useAdminTable(filtered, ["question", "answer", "category"]);

  const openAdd = () => { setForm(emptyForm); setEditingId(null); setModalOpen(true); };
  const openEdit = (item: KnowledgeQA) => { const { id, createdAt, updatedAt, deletedAt, ...rest } = item; setForm(rest); setEditingId(id); setModalOpen(true); };
  const save = () => {
    if (!form.question.trim()) return toast.error("الرجاء إدخال السؤال");
    if (editingId) { updateItem("qa", editingId, form); toast.success("تم التعديل"); }
    else { addItem("qa", form); toast.success("تمت الإضافة"); }
    setModalOpen(false); reload();
  };
  const remove = (id: string) => { softDeleteItem("qa", id); toast.success("تم الحذف"); setDeleteConfirm(null); reload(); };
  const restore = (id: string) => { restoreItem("qa", id); toast.success("تمت الاستعادة"); reload(); };
  ...
  return (
    <div>
      <div className="mb-6 flex items-center justify-between">
        <h1 className="text-2xl font-bold">مكتبة الأسئلة والأجوبة</h1>
        <div className="flex items-center gap-2">
          ...
          <Button onClick={() => exportSectionToJSON("qa")} variant="outline" size="sm"><Download className="h-4 w-4" /> تصدير</Button>
      ...
      <p className="mb-4 text-sm text-muted-foreground">عرض {paged.length} من {total} سؤال</p>
            ...
            <table className="w-full text-sm">
              <thead className="border-b border-border bg-muted/40">
                <tr>
                  ...
                  <th className="px-4 py-3 text-right font-semibold">الأهمية</th>
                ...
                {paged.map((item) => (
                  <tr key={item.id} className={`border-b border-border/50 hover:bg-muted/20 ${item.deletedAt ? "opacity-50" : ""}`}>
                    <td className="px-4 py-3 font-medium max-w-xs truncate">
                      {item.deletedAt ? item.question : <Link href={`/admin/knowledge-center/qa/${item.id}`} className="text-accent hover:underline">{item.question}</Link>}
                    </td>
                    <td className="px-4 py-3 text-muted-foreground">{item.category}</td>
                    <td className="px-4 py-3"><span className={`rounded-full px-2 py-0.5 text-xs font-semibold ${item.importance === "high" ? "bg-red-100 text-red-700" : item.importance === "medium" ? "bg-amber-100 text-amber-700" : "bg-green-100 text-green-700"}`}>{importanceLabels[item.importance]}</span></td>
      ...
      <ConfirmDialog open={deleteConfirm !== null} title="تأكيد الحذف" message="هل أنت متأكد من حذف هذا السؤال؟" onConfirm={() => { if (deleteConfirm) remove(deleteConfirm); }} onCancel={() => setDeleteConfirm(null)} />
      {modalOpen && (
        <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-black/50 p-4" onClick={() => setModalOpen(false)}>
          <Card className="my-8 w-full max-w-2xl" onClick={(e) => e.stopPropagation()}>
            <CardHeader className="flex flex-row items-center justify-between">
              <CardTitle>{editingId ? "تعديل سؤال" : "إضافة سؤال جديد"}</CardTitle>
              <button onClick={() => setModalOpen(false)} className="rounded-lg p-1 hover:bg-muted/60"><X className="h-5 w-5" /></button>
            </CardHeader>
            <CardContent className="grid grid-cols-2 gap-4">
              <div className="col-span-2">
                <label className="mb-1 block text-sm font-medium">السؤال *</label>
                <textarea className="w-full rounded-xl border border-border bg-background px-3 py-2 text-sm outline-none focus:border-accent" rows={3} value={form.question} onChange={(e) => setForm({ ...form, question: e.target.value })} />
              </div>
              <div className="col-span-2">
                <label className="mb-1 block text-sm font-medium">الإجابة *</label>
                <textarea className="w-full rounded-xl border border-border bg-background px-3 py-2 text-sm outline-none focus:border-accent" rows={6} value={form.answer} onChange={(e) => setForm({ ...form, answer: e.target.value })} />
              </div>
              <div>
              ...
              <div>
                <label className="mb-1 block text-sm font-medium">درجة الأهمية</label>
                <select className="w-full rounded-xl border border-border bg-background px-3 py-2 text-sm outline-none focus:border-accent" value={form.importance} onChange={(e) => setForm({ ...form, importance: e.target.value as "high" | "medium" | "low" })}>
