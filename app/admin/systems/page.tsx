...
const emptyItem: Omit<System, "id"> = {
  number: "",
  year: "",
  date: "",
  title: "",
...
const authorities = [
  ...
  { value: "البرلمان", label: "البرلمان" },
  { value: "البنك المركزي العراقي", label: "البنك المركزي العراقي" },
  { value: "هيئة الأوراق المالية", label: "هيئة الأوراق المالية" },
  { value: "الهيئة الاتحادية للنفط", label: "الهيئة الاتحادية للنفط" },
];
...
export default function SystemsAdminPage() {
  ...
  const [form, setForm] = useState<Omit<System, "id">>(emptyItem);
  ...
  const { search, setSearch, page, setPage, pageSize, setPageSize, paged, totalPages, total } = useAdminTable(items, ["number", "title", "issuingAuthority", "scope", "year"], 10);
  ...
  async function save() {
    if (!form.number.trim()) { toast.error("خطأ", "رقم النظام مطلوب"); return; }
    try {
      if (editingId) {
        await fetch(`/api/systems/${editingId}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(form) });
        toast.success("تم الحفظ", "تم تحديث النظام بنجاح");
      } else {
        const res = await fetch("/api/systems", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(form) });
        const created = await res.json();
        setItems((prev) => [created, ...prev]);
        toast.success("تمت الإضافة", "تم إضافة النظام بنجاح");
      }
      setModalOpen(false);
      fetchItems();
  ...
  async function remove(id: string) {
    try {
      ...
      toast.success("تم الحذف", "تم حذف النظام بنجاح");
  ...
  return (
    <div>
      ...
      <div className="mb-4 flex items-center justify-between">
        <p className="text-sm text-muted-foreground">عرض {toArabicDigits(paged.length)} من {toArabicDigits(total)} نظام</p>
      </div>

      <div className="relative mb-4">
        <Search className="absolute right-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
        <input className="w-full rounded-xl border border-border bg-muted/40 pr-10 pl-10 h-11 text-sm outline-none focus:border-accent" placeholder="بحث في الأنظمة..." value={search} onChange={(e) => setSearch(e.target.value)} />
        {search && (
      ...
      <ConfirmDialog open={deleteConfirm !== null} title="تأكيد الحذف" message="هل أنت متأكد من حذف هذا النظام؟ لا يمكن التراجع عن هذا الإجراء." onConfirm={() => remove(deleteConfirm!)} onCancel={() => setDeleteConfirm(null)} />
        ...
        <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-black/50 p-4" onClick={() => setModalOpen(false)}>
          <Card className="my-8 w-full max-w-2xl" onClick={(e) => e.stopPropagation()}>
            <CardHeader className="flex flex-row items-center justify-between">
              <CardTitle>{editingId ? "تعديل نظام" : "إضافة نظام جديد"}</CardTitle>
              <button onClick={() => setModalOpen(false)} className="rounded-lg p-1 hover:bg-muted/60"><X className="h-5 w-5" /></button>
            </CardHeader>
            <CardContent className="grid grid-cols-2 gap-4">
              <div>
                <label className="mb-1 block text-sm font-medium">رقم النظام *</label>
              ...
              <div>
                <label className="mb-1 block text-sm font-medium">تاريخ النشر</label>
              ...
              <div>
                <label className="mb-1 block text-sm font-medium">النطاق</label>
                <input className="w-full rounded-xl border border-border bg-background px-3 py-2 text-sm outline-none focus:border-accent" value={form.scope} onChange={(e) => setForm({ ...form, scope: e.target.value })} placeholder="وطني / محلي / خاص بجهة" />
              </div>
              <div>
              ...
              <div className="col-span-2">
                <label className="mb-1 block text-sm font-medium">عنوان النظام</label>
                <input className="w-full rounded-xl border border-border bg-background px-3 py-2 text-sm outline-none focus:border-accent" value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} />
              </div>
              <div className="col-span-2">
                <label className="mb-1 block text-sm font-medium">نص النظام الكامل</label>
