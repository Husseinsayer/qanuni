...
export default function EditArticlePage() {
  ...
  const existingBodies = data.articleBodies[id] || [];

  const [title, setTitle] = useState(existing?.title || "");
  const [category, setCategory] = useState(existing?.category || categories[0]);
  const [lawyerId, setLawyerId] = useState(existing?.lawyerId || data.lawyers[0]?.id || "");
  const [date, setDate] = useState(existing?.date || new Date().toISOString().split("T")[0]);
  const [readingTime, setReadingTime] = useState(existing?.readingTime || 5);
  const [excerpt, setExcerpt] = useState(existing?.excerpt || "");
  const [hue, setHue] = useState(existing?.hue || hueOptions[0]);
  const [blocks, setBlocks] = useState<ArticleBlock[]>(existingBodies.length > 0 ? existingBodies : [{ p: "" }]);
  ...
  const save = (publishStatus: "draft" | "published") => {
    ...
    const articleData = {
      ...
      author: lawyer?.name || existing.author,
      lawyerId,
      date,
      readingTime,
      hue,
    ...
    update("articles", data.articles.map((a) => (a.id === id ? articleData : a)));
    update("articleBodies", { ...data.articleBodies, [id]: blocks.filter((b) => b.p.trim()) });
    toast.success(publishStatus === "published" ? "تم النشر بنجاح" : "تم الحفظ كمسودة", `تم تحديث المقال "${title}"`);
    ...
    <div>
      <div className="mb-6 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <span className={`rounded-full px-2.5 py-0.5 text-xs font-semibold ${existing.status === "published" ? "bg-green-100 text-green-700" : "bg-amber-100 text-amber-700"}`}>
            {existing.status === "published" ? "منشور" : "مسودة"}
          </span>
