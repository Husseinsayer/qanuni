  ...
  const save = async (publishStatus: string) => {
    ...
    try {
      const res = await fetch(`/api/admin/articles/${id}`, {
        ...
        body: JSON.stringify({
          title: title.trim(), category, authorId, date, readingTime,
          excerpt: excerpt.trim(), hue, content: htmlContent, status: publishStatus,
        }),
      });
      if (res.ok) {
        toast.success("تم الحفظ", "تم تحديث المقال");
            ...
            <CardContent className="p-5">
              ...
              <div className="mb-4 grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select className="w-full rounded-xl border border-border bg-background px-3 py-2 text-sm outline-none focus:border-accent" value={category} onChange={(e) => setCategory(e.target.value)}>
                    {categories.map((c) => <option key={c} value={c}>{c}</option>)}
                  </select>
                </div>
                <div>
                  <label className="mb-1 block text-sm font-medium">التاريخ</label>
                  <input type="date" className="w-full rounded-xl border border-border bg-background px-3 py-2 text-sm outline-none focus:border-accent" value={date} onChange={(e) => setDate(e.target.value)} />
                </div>
                <div>
                  <label className="mb-1 block text-sm font-medium">وقت القراءة</label>
                  <input type="number" className="w-full rounded-xl border border-border bg-background px-3 py-2 text-sm outline-none focus:border-accent" value={readingTime} onChange={(e) => setReadingTime(Number(e.target.value))} />
                </div>
                <div>
              ...
              <div className="mb-4">
                ...
                <textarea className="w-full rounded-xl border border-border bg-background px-3 py-2 text-sm outline-none focus:border-accent" rows={2} value={excerpt} onChange={(e) => setExcerpt(e.target.value)} />
              </div>
